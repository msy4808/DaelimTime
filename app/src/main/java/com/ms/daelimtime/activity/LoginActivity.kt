...
class LoginActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val kakao_Btn = findViewById<ImageButton>(R.id.kakao_login_button)


        // 로그인 버튼
        kakao_Btn.setOnClickListener {
            if (userNickName_edit.getText().toString().trim()
                    .equals("") || userClassNum_edit.getText().toString().trim().equals("")
            ) {
                Toast.makeText(this, "유저 정보를 입력하세요", Toast.LENGTH_SHORT).show()
            } else {
    ...
    private fun getUserData() {
        UserApiClient.instance.me { user, error ->
            if (error != null) {
                ...
                DBHelper.database.child("User").child("UID_${DBHelper.id}").child("userID")
                    .setValue(DBHelper.id)
                UserDataSet()
                //여기까지지
            }
        }
