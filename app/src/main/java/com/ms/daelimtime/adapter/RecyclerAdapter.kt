...
import com.ms.daelimtime.activity.LoadingActivity
...
class RecyclerAdapter(val context: Context?) : RecyclerView.Adapter<ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        DBHelper.database.child("School_Survey").child("SC_${holder.cardTitle.text}").get().addOnSuccessListener {
            when(it.value) {
                null -> {
                    DBHelper.database.child("Student_Survey").child("ST_${holder.cardTitle.text}").get().addOnSuccessListener {
                        holder.del_Btn.setOnClickListener {
                            DBHelper.database.child("Student_Survey").child("ST_${holder.cardTitle.text}").setValue(null)
                            DBHelper.database.child("Result").child("ST_${holder.cardTitle.text}").setValue(null)
                    ...
                    holder.del_Btn.setOnClickListener {
                        DBHelper.database.child("School_Survey").child("SC_${holder.cardTitle.text}").setValue(null)
                        DBHelper.database.child("Result").child("ST_${holder.cardTitle.text}").setValue(null)
            ...
            .child("UID_${DBHelper.id}").get().addOnSuccessListener {
                when (it.value) {
                    null -> {
                            ...
                            .child("UID_${DBHelper.id}").get().addOnSuccessListener {
                                when (it.value) {
                                    null -> {
                                        ...
                                        DBHelper.database.child("Student_Survey")
                                            .child("ST_${title}").child("type").get()
                                            .addOnSuccessListener {
                                                when (it.value) { //A설문 일경우 Result A 로 이동
                                                    "A" -> {
                                                        val intent = Intent(
                                                            context,
                                                            Result_A_Activity::class.java
                                                        )
                                                        intent.putExtra("title", title)
                                                        intent.putExtra("doc", doc)
                                                        if (context != null) { //인탠트 null 검사
                        ...
                        DBHelper.database.child("School_Survey").child("SC_${title}").child("type")
